...
sdir<-"./" # path to the default input files
...
cat("The gene name is the same with counts [1], is coming from IPA [2] or contains some synonyms [3]? \n")
genenames<-as.numeric(readline(prompt="Enter a number: "))
...
if(genenames==2){
  ipa<-read.csv(paste(sdir,"IPA_geneNames.csv",sep=""))
  gn$gene<-unlist(sapply(gn$gene,function(x) ipa[ipa$Symbol==x,"ID"][1]))
}

if(genenames==3){
  #finding the gene synonymw for unknown gene
  syn<-read.csv(paste(sdir,"gene_Symbol_And_Synonyms_mm.csv",sep=""),fill=TRUE,header=FALSE,stringsAsFactors = FALSE)
  syn[] <- lapply(syn,tolower)
  gsynfinder<-function(g,des){
    p<-which(syn==tolower(as.character(g)),arr.ind = TRUE)
    s<-as.character(unlist(syn[p[1],]))
    s<-s[! is.na(s) & ! s==""]  
    res<-des[which(tolower(des) %in% s)]
    if(length(res)==0){return(g)} else {return(res[1])}
  }
  
  p<-which(! gn$gene  %in% rownames(counts))
  unrsyn<-sapply(gn$gene[p],gsynfinder,rownames(counts))
  gn$gene[p]<-unrsyn
